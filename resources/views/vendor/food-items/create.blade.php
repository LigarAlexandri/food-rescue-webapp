<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ isset($foodItem) ? 'Edit Food Item' : 'List New Food Item' }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ isset($foodItem) ? route('vendor.food-items.update', $foodItem->id) : route('vendor.food-items.store') }}" enctype="multipart/form-data">
                        @csrf
                        @if(isset($foodItem))
                            @method('PUT')
                        @endif
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="pickup_location" class="block mt-1 w-full" type="text" name="pickup_location" :value="old('pickup_location', $foodItem->pickup_location ?? Auth::user()->address ?? '')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="pickup_start_time" class="block mt-1 w-full" type="datetime-local" name="pickup_start_time" :value="old('pickup_start_time', isset($foodItem) ? ($foodItem->pickup_start_time ? $foodItem->pickup_start_time->format('Y-m-d\TH:i') : '') : '')" required />
                            <x-input-error :messages="$errors->get('pickup_start_time')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="pickup_end_time" :value="__('Pickup End Time')" />
                            <x-text-input id="pickup_end_time" class="block mt-1 w-full" type="datetime-local" name="pickup_end_time" :value="old('pickup_end_time', isset($foodItem) ? ($foodItem->pickup_end_time ? $foodItem->pickup_end_time->format('Y-m-d\TH:i') : '') : '')" required />
                        ...
                        <div class="mt-4">
                            ...
                            <select name="status" id="status" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm">
                                ...
                                {{-- Vendor cannot directly set to claimed, completed, expired from edit form --}}
                            </select>
                        ...
                        <div class="flex items-center justify-end mt-6">
                            ...
                            <x-primary-button>
                                {{ isset($foodItem) ? __('Update Item') : __('List Item') }}
                            </x-primary-button>
                        </div>
                    </form>
                </div>
